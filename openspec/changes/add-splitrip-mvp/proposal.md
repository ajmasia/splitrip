...
- **An account to open a trip, none to join one**: creating a trip requires signing in with an email address and a password. Public sign-up is deliberately absent — accounts are arranged out of band — so the number of people who can open new trips on an instance stays bounded, and a personal deployment does not quietly become somebody else's free hosting.
