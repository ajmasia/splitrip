...
- [x] 2.6 Create the balances view aggregating paid, charged and settled per participant; verify against a fixture data set that the balances match hand-computed figures and sum to exactly zero
...
- [x] 3.4 Implement the interface amount formatting and parsing; verify with tests that it rejects amounts with more than two decimal places, zero and negatives, and that it rounds correctly when converting to cents
...
- [x] 4.4 Implement the join-by-invitation RPC function, including the duplicate-name check and rejoining from a second device; verify with tests the valid, revoked, expired and closed-trip invitation cases
- [x] 4.5 Implement the close- and reopen-trip RPC functions, generating the JSONB summary snapshot on close; verify with tests that the summary does not change between two reads and that it is regenerated on reopening and closing again
- [x] 4.6 Add the closed-trip check to every write function; verify with tests that any write against a `closed` trip is rejected

## 5. Identity and access

- [x] 5.1 Integrate the Supabase client with an automatic anonymous session on first visit and token refresh; verify that a fresh browser gets a stable `auth.uid()` that survives reloads
...
- [x] 5.10 Allow an `admin` to add a participant by name alone, with no invitation and no session of their own: make the session on a participant optional, add the function that creates one and the form that calls it; verify that the added participant appears in the list, can be included in a split and be named as the payer of an expense, that a duplicate name is rejected, and that a `participant` cannot add anybody
- [x] 5.11 Let somebody added that way take over their own place through an invitation, keeping their role, expenses and balance, and tell that case apart from a name already in use by a device; verify with tests both messages, that the balance after binding is the one the participant already had, and that nothing is duplicated
- [x] 5.12 Add the invitation that opens one place: a button beside each participant that mints a link for them and shows it there as a link and a QR, taking its role from the place, reusing the live one, and letting them come in with a single action and no name; verify that the link binds to the participant that already exists, that a second holder is refused, that asking twice hands back the same link, and that a participant of another trip cannot be named

## 6. Interface foundation: PWA and languages

- [x] 6.1 Implement the Spanish and English copy catalogues with typed keys; verify that the compiler fails when a key exists in Spanish and is missing from English
- [x] 6.2 Implement language resolution (stored preference, browser header, Spanish by default) and the persistent language switcher; verify the three resolution paths and that the preference survives a reload
- [x] 6.3 Implement amount and date formatting with `Intl` according to the active language; verify that 1055 cents render as "10,55 €" in Spanish and with English conventions in English
- [x] 6.4 Implement the phone base layout with thumb-reachable navigation and touch targets of at least 44 pixels; verify on a 360-pixel-wide screen that there is no horizontal scrolling
...
- [x] 7.2 Implement the add-expense form with its default values (payer is whoever records it, split among everyone, today's date); verify that an expense can be recorded providing only a description and an amount
- [x] 7.3 Add payer selection for an `admin` and split-subset selection for every participant to the form; verify that a €45.00 expense split among three of five charges €15.00 to each and leaves the other two unchanged, and that a `participant` naming somebody else as the payer is rejected
