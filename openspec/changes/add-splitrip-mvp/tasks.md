...
- [ ] 2.1 Create the migration with the `trips`, `participants` and `invitations` tables, with their keys, uniqueness constraints and the role `CHECK`; verify that the migration applies to the local database and that the constraints reject the invalid cases
- [ ] 2.2 Create the migration with the `expenses`, `expense_shares` and `payments` tables, with amounts as `BIGINT` cents, a `currency` column with `CHECK (currency = 'EUR')`, a positive-amount `CHECK` and the constraint preventing a `contribution` from having shares; verify with inserts that each constraint fires
