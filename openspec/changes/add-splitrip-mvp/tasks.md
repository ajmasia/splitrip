...
- [x] 5.8 Put the account barrier in front of creating a trip and turn off public sign-up; verify with tests that a session holding only a device identity is refused, that a signed-in one succeeds, and that joining by invitation and being an `admin` through one are unaffected
